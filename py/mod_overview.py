...
def printStats(gl , soup):
	...
	try:
		...
		yesterday = (soup.find("div", { "id" : "postviews" }, { "class" : "postbox" }).find("div", { "class" : "yesterday" }).find("tr", {"class": "total"}).find("td", {"class" : "views"}).string).encode('utf-8')
	...
	try:
		...
		print('$font'+gl.textCol+ 'Yesterday' + ' $alignr ' + gl.numCol + yesterday)
